---
session: BMS-3930 split into Ph 1 + Ph 2
...
decisions_locked_in:
  - "Multi-entity model = multiple Account records per retailer (option B)"
  - "Ph 1 ships read-view + manual AR entry; Ph 2 ships AR sync + invoice list"
  - "Portal placement = dedicated Payment Status page"
  - "Ph 1 cuts Last Payment and Outstanding Invoices (move to Ph 2)"
...
tags: [polish, ecom, gulf, credit, split, jira-ready]
---

# BMS-3930 — Ph 1 / Ph 2 split drafts (2026-05-13)

This note holds two Jira-ready ticket drafts produced from today's polish session:

1. **BMS-3930 (Ph 1)** — revised in place, targets Sprint 2 (2026-05-04 → 2026-05-15)
2. **New ticket (Ph 2)** — to be created, targets a future sprint

Both reuse the structural skeleton from [[BMS-3930-credit-terms-ph1-compact]] (2026-04-23) and fold in the field-name + multi-entity decisions from [[BMS-3930-credit-terms-polish-2026-05-13]].

---

## Draft 1 — BMS-3930 (Ph 1, this sprint)

**Title:** Retailer credit terms display — Ph 1 read view (Gulf)

**Type:** Story · **Sprint:** 2 · **Story points:** L (probably trim toward M after this rewrite) · **Parent:** BMS-3702
...
### Technical notes (replaces auto-generated approach)

- **Data model:** all 6 new fields go on `Account` in `OHFY-Data-Model`. No new custom object.
...
## Draft 2 — New ticket (Ph 2, future sprint)

**Title:** Retailer credit terms — Ph 2: AR sync, invoice list & payment history (Gulf)

**Type:** Story · **Sprint:** future (depends on Gulf AR integration timeline) · **Parent:** BMS-3702
...
- **Sync mechanism:** depends on Gulf's AR system. Options to evaluate: REST callout (Named Credential + Apex), Salesforce-side scheduled batch pulling, or AR-system-side push via Platform Event / Connected App. Decision lives in a spike before this story is started.
...
- **Derivation:** `Past_Due_Balance__c` becomes a rollup of `Invoice__c` records where `Payment_Due_Date__c < TODAY() AND Payment_Status__c IN ('Not Paid', 'Partially Paid')`. `Outstanding_Balance__c` becomes a rollup of all `Invoice__c.Outstanding_Amount__c` (or equivalent) for the Account.
...
1. **Update BMS-3930 in place** with Draft 1 — title rename to "Retailer credit terms display — Ph 1 read view (Gulf)", replace description + ACs, append audit-trail comment referencing these polish notes.
2. **Create new Jira ticket** with Draft 2 — title "Retailer credit terms — Ph 2: AR sync, invoice list & payment history (Gulf)", parent BMS-3702, blocked-by BMS-3930.
...
I'll do all four as one step when you give the go.
