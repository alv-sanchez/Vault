...
Clean up the following and get `hardcoded-color-allowlist.md` out of the scope of ecommerce

CSS (stylelint):
  /* stylelint-disable ohfy-theme/no-hardcoded-hex */
  .selector { color: #f0fdf4; }
  /* stylelint-enable ohfy-theme/no-hardcoded-hex */

  HTML (custom linter):
  <!-- ohfy-disable no-hardcoded-hex -->
  <div style="color: #15803d">...</div>
  <!-- ohfy-enable no-hardcoded-hex -->
  
  JS (eslint):
  // eslint-disable-next-line ohfy-theme/no-hardcoded-hex
  const color = "#e53e3e";

  Key difference: the HTML linter's <!-- ohfy-disable-next-line no-hardcoded-hex --> only covers the very next line, so for multi-line style=""
  attributes you need the block disable/enable pair instead.